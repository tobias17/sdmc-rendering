...
import gzip, argparse, math, re, cv2, json
...
if __name__ == "__main__":
   ...
   # load in weights
   # load_state_dict(model, torch_load(fetch('https://huggingface.co/CompVis/stable-diffusion-v-1-4-original/resolve/main/sd-v1-4.ckpt', 'sd-v1-4.ckpt'))['state_dict'], strict=True, fp16=args.fp16)
   ...
   #       segments[-1].append([])
   #       for x in range(sz):
   #          ch = img[y*delta:(y+1)*delta,x*delta:(x+1)*delta,0]
   #          m = stats.mode(img[y*delta:(y+1)*delta,x*delta:(x+1)*delta,0].flatten())[0]
